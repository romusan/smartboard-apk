...
<style>html,body{{margin:0;height:100%;background:#08111f;color:#eaf2ff;font:14px system-ui}}header{{padding:10px 14px;background:#10213a}}button{{margin:4px;padding:8px 12px;border:0;border-radius:10px;background:#2563eb;color:white}}canvas{{display:block;width:100%;height:calc(100% - 92px)}}.muted{{color:#9fb2cb}}</style></head>
<body><header><b>Simulación de mecanismos · {safe_id}</b><div class='muted'>Curva objetivo + solución cinemática. RMS normalizado: {rms:.4f}</div>
<button onclick="family='fourbar'">4 barras · PSO‑TASS</button><button onclick="family='watt'">6 barras · Watt I</button><button onclick="family='stephenson'">6 barras · Stephenson III</button></header><canvas id='c'></canvas>
<script>const target={target_json},p={params_json};let family='fourbar',t=0;const c=document.getElementById('c'),x=c.getContext('2d');
...
function pt(q,s,ox,oy){{return [ox+q[0]*s,oy-q[1]*s]}}function line(a,b,color,w=5){{x.strokeStyle=color;x.lineWidth=w;x.beginPath();x.moveTo(...a);x.lineTo(...b);x.stroke()}}function joint(a){{x.fillStyle='#f8fafc';x.beginPath();x.arc(a[0],a[1],7,0,7);x.fill();x.strokeStyle='#0f172a';x.stroke()}}
function loop(){{x.clearRect(0,0,c.width,c.height);const S=Math.min(c.width,c.height)*.36,ox=c.width*.5,oy=c.height*.52;x.strokeStyle='#38bdf8';x.lineWidth=4;x.beginPath();target.forEach((q,i)=>{{const a=pt(q,S,ox,oy);i?x.lineTo(...a):x.moveTo(...a)}});x.closePath();x.stroke();
const g=p[0],a=p[1],b=p[2],d=p[3],ang=p[6]+t,A=[0,0],D=[g,0],B=[a*Math.cos(ang),a*Math.sin(ang)],dx=D[0]-B[0],dy=-B[1],L=Math.hypot(dx,dy),al=(b*b-d*d+L*L)/(2*L),hh=Math.sqrt(Math.max(0,b*b-al*al)),C=[B[0]+al*dx/L-hh*dy/L,B[1]+al*dy/L+hh*dx/L];let nodes=[A,B,C,D];if(family!=='fourbar'){{const E=[B[0]+.62*(C[0]-B[0])-.55*(C[1]-B[1]),B[1]+.62*(C[1]-B[1])+.55*(C[0]-B[0])],F=family==='watt'?[g*.15,-.75]:[g*.82,-.7];nodes.push(E,F);line(pt(E,S,ox,oy),pt(F,S,ox,oy),'#f59e0b',10);line(pt(C,S,ox,oy),pt(E,S,ox,oy),'#a78bfa',10)}}line(pt(A,S,ox,oy),pt(B,S,ox,oy),'#ef4444',10);line(pt(B,S,ox,oy),pt(C,S,ox,oy),'#22c55e',10);line(pt(C,S,ox,oy),pt(D,S,ox,oy),'#f59e0b',10);line(pt(A,S,ox,oy),pt(D,S,ox,oy),'#64748b',12);nodes.forEach(q=>joint(pt(q,S,ox,oy)));t+=.025;requestAnimationFrame(loop)}}loop();</script></body></html>"""
